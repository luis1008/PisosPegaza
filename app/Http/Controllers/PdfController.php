...
{
    public function pdf_pedido($id,$preorden = 0,$copia = 0){
    	$leyenda   = ($preorden != 0) ? "PREORDEN" : "ORDEN";
    	$tipo_nota = ($copia != 0) ? "COPIA" : "ORIGINAL";
    	$ped = Pedido::find($id);
    ...
    public function ticket_viaje($id, $copia = 0){
    	...
    	$viaje = Viaje::find($id);
        $pdf = \PDF::loadview('Pdf.Modulos.ticket_viaje',['viaje'=>$viaje,'nota'=>$tipo_ticket]);
        return $pdf->stream();
