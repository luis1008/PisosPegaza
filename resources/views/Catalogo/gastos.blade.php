        ...
        <table class="table table-hover table-sm">
            ...
            <tbody>
                ...
                <?php foreach ($gastos as $gasto) { ?>
                    <tr class='<?php if($gasto->catga_status){ echo "table-success"; } else { echo "table-danger"; } ?>'>
                        <th class="text-center"><?php echo str_pad($gasto->id_cat_gastos, 2, "0", STR_PAD_LEFT) ?></th>
                        <td><?php echo $gasto->catga_gastos ?></td>
                        <td>
                            <button type="button" class="btn btn-dark btn-sm tooltips2" title="Modificar" data-toggle="modal" data-target="#mo-{{$gasto->id_cat_gastos}}"><span class="icon icon-pencil"></span></button>

                            <?php if($gasto->catga_status){ ?>
                                <a href="{{route('put_gasto',$gasto->id_cat_gastos)}}" class="btn btn-danger btn-sm tooltips2" title="Suspender"><span class="icon icon-arrow-down"></span></a>
                            <?php } else { ?>
                                <a href="{{route('put_gasto',$gasto->id_cat_gastos)}}" class="btn btn-danger btn-sm tooltips2" title="Activar"><span class="icon icon-arrow-up"></span></a>
                            <?php } ?>
                        </td>
                    </tr>
                    <!-- MODIFICAR GASTO -->
                    <div class="modal fade" id="mo-{{$gasto->id_cat_gastos}}" tabindex="-1" role="dialog" aria-labelledby="vehiculos" aria-hidden="true">
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form action="<?php echo route('put_datos_gasto',$gasto->id_cat_gastos)?>" method="POST">
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col">
                                                <div class="input-group">
                                                    ...
                                                    <input type="text" class="form-control" name="concepto" value="{{$gasto->catga_gastos}}" placeholder="CONCEPTO" required>
                                                </div>
                                            </div>
                                        </div>
